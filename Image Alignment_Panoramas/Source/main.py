...
# ===================================================
# ================ Perspective Warping =======.0=======
# ===================================================
def Perspective_warping(img1, img2, img3):

	# Write your codes here
	img1 = cv2.copyMakeBorder(img1,200,200,500,500, cv2.BORDER_CONSTANT)                                                  #creating space for adding image 2 and image 3
	(M, pts1, pts2, mask) = getTransform(img2, img1,'homography')                                                         # getting the homographic transform for image 2 with respect to image 1
	img1 = cv2.warpPerspective(img2,M, (img1.shape[1],img1.shape[0]), dst=img1.copy(), borderMode=cv2.BORDER_TRANSPARENT) #converting image 2 to plane of image 1
	(M, pts1, pts2, mask) = getTransform(img3,img1,'homography')	                                                      # getting the homographic transform for image 2 with respect to image 1
	output_image = cv2.warpPerspective(img3,M, (img1.shape[1],img1.shape[0]), dst=img1.copy(), borderMode=cv2.BORDER_TRANSPARENT)  #converting image 3 to plane of image 1
...
def Bonus_perspective_warping(img1, img2, img3):
    ...
    #applying borders to image 1 ,image 2,image 2
    ...
    #taking image 1 as anchor image
    imgpers1 = img1  
                                 
                 
    (M, pts1, pts2, mask) = getTransform(img2, img1, 'homography')           # getting the homographic transform for image 2 with respect to image 1
 
    imgpers2 = cv2.warpPerspective(img2, M, (img1.shape[1], img1.shape[0]))  #converting image 2 to plane of image 1
 
    (M, pts1, pts2, mask) = getTransform(img3, img1, 'homography')           # getting the homographic transform for image 2 with respect to image 1
 
    imgpers3 = cv2.warpPerspective(img3, M, (img1.shape[1], img1.shape[0]))  #converting image 3 to plane of image 1
 
   
    #applying borders to wrap perspectives of image 1 ,image 2,image 2

    imgpers1 = cv2.copyMakeBorder(imgpers1, 160, 160, 12, 12, cv2.BORDER_CONSTANT)
    imgpers2 = cv2.copyMakeBorder(imgpers2, 160, 160, 12, 12, cv2.BORDER_CONSTANT)
    imgpers3 = cv2.copyMakeBorder(imgpers3, 160, 160, 12, 12, cv2.BORDER_CONSTANT)
 
    #Calling out the laplacian blending function
    img_out = BlendImages(imgpers1, imgpers2, imgpers3, 0.43, 0.555)          
 
    #resizing the blended image back to original image size
    cols, rows = img_out.shape[0]-imgpers1.shape[0], img_out.shape[1]-imgpers1.shape[1]         
    output_image = img_out[cols/2:imgpers1.shape[0]+(cols/2), rows/2:imgpers1.shape[1]+(rows/2)]
...
def Cylindrical_warping(img1, img2, img3):
	...
	img1 = cv2.copyMakeBorder(img1,50,50,300,300, cv2.BORDER_CONSTANT)
        ...
        #Getting cylindrical wrap of image 1
        h,w = img1.shape
    	f = 497
    	K = np.array([[f, 0, w/2], [0, f, h/2], [0, 0, 1]]) # mock calibration matrix
    	imcyl1, imcylmask1 = cylindricalWarpImage(img1, K)

        #Getting cylindrical wrap of image 2
	h,w = img2.shape
    	f = 497
    	K = np.array([[f, 0, w/2], [0, f, h/2], [0, 0, 1]]) # mock calibration matrix
	...
	(M, pts1, pts2, mask) = getTransform(imcyl2, imcyl1 , 'affine')  # getting the affine transform for image 2 with respect to image 1
	img_in2 = cv2.warpAffine(imcyl2, M, (imcyl1.shape[1],imcyl1.shape[0])) #converting image 2 to plane of image 1

        #Getting cylindrical wrap of image 3
	h,w = img3.shape
    	f = 497
    	K = np.array([[f, 0, w/2], [0, f, h/2], [0, 0, 1]]) # mock calibration matrix
    	imcyl3, imcylmask3  = cylindricalWarpImage(img3, K)

	(M, pts1, pts2, mask) = getTransform(imcyl3, imcyl1 , 'affine')         # getting the affine transform for image 3 with respect to image 1
	img_in3 = cv2.warpAffine(imcyl3, M, (imcyl1.shape[1],imcyl1.shape[0]))  #converting image 3 to plane of image 1

	img1mask = np.ma.masked_not_equal(imcyl1, 0)               #Applying mask to filter out the zero values in cylindrical image 1
	output_image = img1mask+img_in2+img_in3                    #Stiching image 1,image 2 and image 3 on the zero pixels of image 1
...
def Bonus_cylindrical_warping(img1, img2, img3):
    ...
    #Getting cylindrical wrap of image 12
    h, w = img2.shape
    f = 497
    K = np.array([[f, 0, w / 2], [0, f, h / 2], [0, 0, 1]])  # mock calibration matrix
    imcyl2, imcylmask2 = cylindricalWarpImage(img2, K)
    ...
    (M, pts1, pts2, mask) = getTransform(imcyl2, imcyl1, 'affine')          # getting the affine transform for image 2 with respect to image 1
    imgaffine2 = cv2.warpAffine(imcyl2, M, (imcyl1.shape[1], imcyl1.shape[0]))  #converting image 2 to plane of image 1
    ...
    (M, pts1, pts2, mask) = getTransform(imcyl3, imcyl1, 'affine')          # getting the affine transform for image 3 with respect to image 1
    imgaffine3 = cv2.warpAffine(imcyl3, M, (imcyl1.shape[1], imcyl1.shape[0]))  #converting image 3 to plane of image 1
 

    imgaffine1 = imcyl1

    #applying borders to wrap perspectives of image 1 ,image 2,image 2
    imgaffine1= cv2.copyMakeBorder(imgaffine1, 294, 294, 196, 196, cv2.BORDER_CONSTANT)
    imgaffine2 = cv2.copyMakeBorder(imgaffine2, 294, 294, 196, 196, cv2.BORDER_CONSTANT)
    imgaffine3 = cv2.copyMakeBorder(imgaffine3, 294, 294, 196, 196, cv2.BORDER_CONSTANT)
 
    #Calling out the laplacian blending function
    img_out = BlendImages(imgaffine1, imgaffine2, imgaffine3, 0.43, 0.555)
 
    #resizing the blended image back to original image size
    cols, rows = img_out.shape[0]-imcyl1.shape[0], img_out.shape[1]-imcyl1.shape[1]
    output_image= img_out[cols/2:imcyl1.shape[0]+(cols/2), rows/2:imcyl1.shape[1]+(rows/2)]
...
def BlendImages(img1, img2, img3, ratio1, ratio2):
    ...
    for laplacianA, laplacianB, laplacianC in zip(lp1, lp2, lp3):
        ...
        # laplacianComb = np.hstack((laplacianA[:, 0:0.5 * cols], laplacianB[:, 0.5 * cols:]))
...
if __name__ == '__main__':
   ...
   #input_image4 = cv2.imread(sys.argv[5], 0) 
   #input_image5 = cv2.imread(sys.argv[6], 0) 
   ...
   #function_launch[question_number](question_number, input_image4, input_image5)
